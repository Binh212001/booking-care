<template>
  <div>
    <div class="medicine-edit-page">
      <Toast />
      <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold">Chỉnh sửa thuốc</h1>
      </div>
...
onMounted(async () => {
  try {
    loading.value = true;
    const medicine = await medicineStore.fetchMedicineById(id);
    if (medicine) {
      initialValues.value = {
        name: medicine.name || "",
        code: medicine.code || "",
        ...
        isEyeMedication: medicine.isEyeMedication ?? false,
...
const onFormSubmit = async ({
  ...
  try {
    if (valid) {
      ...
      await medicineStore.updateMedicine(id, submitData as any);
      navigateTo("/medicine");
    }
  } catch (error) {
